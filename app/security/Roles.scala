...
object Roles {
  ...
  trait CsrAuthorization {
    def isAuthorized(user: CachedData)(implicit request: Request[_]): Boolean

    def isAuthorized(user: CachedDataWithApp)(implicit request: Request[_]): Boolean =
  ...
  object NoRole extends CsrAuthorization {
    override def isAuthorized(user: CachedData)(implicit request: Request[_]) = true
  }
  ...
  def getLatestJourneyStep(user: CachedDataWithApp)(implicit request: Request[_], lang: Lang): Call =
